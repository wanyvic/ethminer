...
URI::URI(const std::string uri)
{
    ...
    if (userpass_flag)
    {
        ...
        auto p = m_user.find_first_of(".");
        if (p != std::string::npos)
        {
            // There should be at least one char after dot
            // returned p is zero based
            if (p < (m_user.length() - 1))
                m_worker = m_user.substr(p+1);

            m_user = m_user.substr(0, p);
        }
        m_user = urlDecode(m_user);
        m_worker = urlDecode(m_worker);
